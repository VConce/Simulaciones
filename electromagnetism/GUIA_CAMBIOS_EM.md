...
No editar `simulador_campo_electrico_compacto_Antigravity.html` salvo que el usuario lo pida explicitamente. Ese archivo queda como referencia externa. La version activa debe estar directamente en `SimulaciÃ³nElecMag`, no dentro de `Versiones`, para que `assets/electric/...` resuelva correctamente.

## Regla de versionado

Para cada cambio funcional:

1. Identificar la version activa en `SimulaciÃ³nElecMag`.
2. Mover o copiar la version activa anterior a `Versiones`.
3. Crear una copia nueva activa en `SimulaciÃ³nElecMag` con el siguiente numero:
   - `EM_Codex_3.1.html`
   - `EM_Codex_3.2.html`
   - `LaboratorioEM.html`
...
La copia compacta debe vivir junto a la version activa en `SimulaciÃ³nElecMag`. Su objetivo es poder enviar un solo HTML con los PNG locales incrustados. Si MathJax sigue cargando desde CDN, indicarlo claramente porque las formulas LaTeX dependen de esa libreria externa.
...
- `ARQUITECTURA_EM.md`.
- La version activa HTML directamente dentro de `SimulaciÃ³nElecMag`.
...
```powershell
Get-ChildItem -Path "C:\Users\victo\Documents\New project\SimulaciÃ³nElecMag\Versiones" | Sort-Object Name
```

Listar assets:

```powershell
Get-ChildItem -Path "C:\Users\victo\Documents\New project\SimulaciÃ³nElecMag\assets\electric"
```

Buscar funciones relevantes:

```powershell
rg -n "function fieldAt|function bAt|function magneticVectorAt|function render|function openMenu|function showContextSelector" "C:\Users\victo\Documents\New project\SimulaciÃ³nElecMag\Versiones\EM_Codex_2.0.html"
```
