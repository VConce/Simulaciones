...
- El fondo activo debe ser `assets/dos-observadores-station-side-bg-source.png`, con vias horizontales claras para que el tren lateral parezca apoyado en los railes.
- Los paneles `Reloj de la estacion` y `Reloj en el tren` deben aparecer apilados uno debajo del otro para facilitar la lectura.
- El sprite del tren tiene mucho margen transparente inferior; la posicion visual correcta usa el contenedor `.trainGroup` bajo (`bottom` pequeno) para que las ruedas visibles queden sobre las vias.
- Maxwell, el reloj de estacion y los recuadros de tiempo deben tener `z-index` superior al tren para que este pase por detras.
- Einstein debe estar visualmente sobre el tren, dentro de `.trainGroup`, no en el anden.
- Hay dos relojes analogicos con esfera raster `observador-clock-sprite.png` y manecillas HTML superpuestas: el de estacion usa `t`; el del tren viaja con `.trainGroup` y usa `t/gamma`.
- El panel lateral incluye `Reiniciar relojes`, que pone `t = 0` y reinicia las manecillas analogicas.
- Desde la version 1.5, el tren se muestra un 50% mas grande que en 1.4 (`width: 1620px` en escritorio). Por el margen transparente del sprite, `.trainGroup` queda con `bottom` negativo para mantener las ruedas sobre las vias; no subirlo sin comprobar visualmente.
- Desde la version 1.6, Einstein se reduce un 30% respecto a 1.5 para quedar proporcionado sobre el tren grande, manteniendo la posicion vertical sobre el techo.
...
- Usa `assets/gemelos-space-bg.png`, `assets/gemelos-earth-twin-sprite.png`, `assets/gemelos-earth-twin-older-sprite.png`, `assets/gemelos-earth-twin-elder-sprite.png`, `assets/gemelos-astronaut-twin-sprite.png` y `assets/gemelos-rocket-sprite.png`.
- El gemelo terrestre debe envejecer visualmente mediante capas de sprites con opacidad interpolada segun el tiempo acumulado en la Tierra.
