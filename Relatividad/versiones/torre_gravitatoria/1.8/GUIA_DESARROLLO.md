...
## Simulaciones nuevas en carpetas propias

`LenteGravitacional/`

- Version inicial `1.0`. Usa canvas y el sprite local de la Tierra para representar al observador.
- El usuario puede arrastrar estrellas fuente y el observador; la masa del agujero negro cambia la curvatura visual de los rayos.
...
- Controles de masa por `kg`, `Mearth` y `Msun`, radio editable y presets Luna/Tierra/Sol/Gargantua.

`ObservadorPaseando/`

- Version inicial `1.0`. Dos escenas sincronizadas; Galileo ve simultaneidad y Einstein ve desfase dependiente de la velocidad.
- El ciclo se reinicia tras las explosiones de las dos estrellas.

- `fmtDelta(sec)` debe escalar automaticamente: hasta `1000 s` muestra segundos; despues minutos hasta `1000 min`; horas hasta `100 h`; dias hasta `500 dias`; anos hasta `300 anos`; y a partir de ahi siglos.
