...
- Si el fondo generado produce saltos de luminosidad en los bordes, usar una version procedimental o procesada con continuidad exacta izquierda-derecha; la prioridad es que el desplazamiento horizontal no revele corte.
...
- El deslizador de velocidad va debajo del boton visible `Reiniciar`, no en la cabecera. El numero `0.95 c` del extremo derecho del control debe permitir introducir velocidad manual entre `0` y `1` mediante texto.
